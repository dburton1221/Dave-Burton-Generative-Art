...
void draw(){
  ...
  int choice = int(random(1,9)); //we have three options
  ...
  if(choice == 1){
    ...
    //make a random shape 
    //try to figure out a way to make this have a random number of vertices
    
    stroke(random(255), random(255), random(255)); //this accidentally changed the parameters in the other shapes, but it is kind of cool
    strokeWeight(random(width/width,width/100));
    fill(random(255), random(255), random(255));
    
    beginShape(); //figure out how to make this a randomly sized loop
    n_times = float(random(3,25));
    randomVertices(n_times);
    endShape(CLOSE);
