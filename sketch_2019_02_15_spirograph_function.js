...
function draw() {
  new_s = second();
  background(random(255),random(255),random(255),random(10,20));
  spirograph();
...
function spirograph(){
  ...
  for (var i = 0; i<sines.length; i++) {
    ...
    stroke(random(255), random(255), random(255),alpha); //this accidentally changed the parameters in the other shapes, but it is kind of cool
    strokeWeight(random(width/width,width/100));
    if (trace) rect(spir_x, spir_y, erad, erad); // draw with erad if tracing
...
function waves(){
  ...
  for (let x = 0; x <= width; x = x + 30) {
    for (let y = 0; y <= height; y = y + 30) {
      ...
      noStroke();
      fill(random(255),random(255),random(255),random(15,26));
