...
float theta, start_pos, stroke_wght; //variables for tree_fractal, https://processing.org/examples/tree.html
...
SpaceShapes[] asteroids; //variables at the top
Herd herd;
EggRing[] eggs; //learning to make composite objects
...
void setup(){
  ...
  eggs = new EggRing[5]; //this goes inside of setup of eggs
  for(int i=0; i<5; i++){ //loop to make the new star objects
  eggs[i]= new EggRing(random(0.25*width,0.75*width), random(0.25*height,0.75*height), random(20,50), random(10,500));}
  
  stars = new Shootingstars[200]; //this goes inside of setup for stars
  for(int i=0; i<200; i++){ //loop to make the new star objects
  stars[i]= new Shootingstars();}
  
  asteroids = new SpaceShapes[100]; //this goes inside of setup
  for(int i=0; i<100; i++){ //loop to make the new asteroid objects
  asteroids[i]= new SpaceShapes();}
...
void draw(){
  ...
  //randomShapeSelector();
  //movingParallelLines();
  //movingRandomFlower(45);
  //moveStars();
  magicCircles(random(0,width), random(0,height), random(width/5));
  //magicRectangles(random(0,width), random(0,height), random(width/5));
  //herd.run();
  //start_tree_fractal(height/3, width*0.01, height); //each one of these makes a progressive translation
  //start_tree_fractal(height/3, width*0.50, height/3); //so, you have to gradually lower each one
  //start_tree_fractal(height/3, width*1, height/3);
  //randomFilter();
  //moveAsteroids();
  moveEggs();
...
//credit to https://processing.org/examples/compositeobjects.html
class Egg {
...
class Ring {
  ...
  void display() {
    if (on == true) {
      noFill();
      strokeWeight(random(width/1000,width/150));
...
class Toid {
  ...
  void render() {
    ...
    fill(RColor, GColor, BColor,hAlpha);
    stroke(random(255),random(255),random(255),hAlpha);
    pushMatrix();
...
//idea from https://funprogramming.org/58-Travel-through-space-use-an-array-to-move-stars.html
class SpaceShapes { //when moving this over into the official script, you must have the class before the functions
 ...
 SpaceShapes(){
    x = random(width);
    y = random(height);
    diameterx = random(width/100, width/10);
    diametery = random(height/100, height/10);
    speedx = random(5,15);
    speedy = random(5,15);
    colorR = random(255);
    colorG = random(255);
...
void moveAsteroids() {
  for (SpaceShapes asteroid : asteroids) {
    asteroid.move_now();
    asteroid.display_now();
  }
}

void moveEggs() {
  for (EggRing egg : eggs) {
    egg.transmit();
  }
}
...
void magicRectangles(float mx, float my, float msz){
  ...
  if(msz>1){
    for(int i=0; i<loopsie; i++){
      ...
      if(the_choice==1){
        innerx = mx + msz/2 * cos(innera);
        innery = my + msz/2 * sin(innera);
      }
      if(the_choice==2){
        innerx = mx + msz/2 * sin(innera);
        innery = my + msz/2 * tan(innera);
      }
      if(the_choice==3){
        innerx = mx + msz/2 * tan(innera);
        innery = my + msz/2 * cos(innera);
      }
...
//TODO, make it so that this accepts width and heigh tvariables,
//b/c then you could draw a whole forest with many trees
//credit to https://processing.org/examples/tree.html
void start_tree_fractal(float hgt, float width_shift, float h_correction){
  stroke(random(255),random(255),random(255));
  stroke_wght = random(1,50); //variable for tree fractal
  strokeWeight(stroke_wght);
  // Let's pick an angle 0 to 90 degrees based on the mouse position
  float a = ( start_pos / (float) width) * 90f;
  // Convert it to radians
  theta = radians(a);
  // Start the tree from the bottom of the screen
  translate(width_shift,h_correction);
...
void randomFilter(){
  //a function that randomly generates a shape
  int choice = int(random(0,7)); //we have three options
  if(choice==0){
    ...
    filter(POSTERIZE,int(random(2,50)));
  } else if(choice==4){
    filter(BLUR,int(random(1,7)));
  } else if(choice==5){
    filter(ERODE);
  } else if(choice==6){
    filter(DILATE);
